...
def train(train_loader, model, vgg_feature_extractor, loss_mse, loss_ssim, optimizer, scheduler, writer, epoch):
    ...
    for data in train_loader:
        ...
        loss3 = 0.005 * vgg_loss_calc(predictImg.to('cuda:1'), RDCTImg.to('cuda:1'), vgg_feature_extractor).to('cuda:0')
        # loss = loss1 + loss2 * 300
        loss = loss3 + loss1

        loss_mse_scalar.update(loss1.item(), LDCTImg.size(0))
        loss_vgg_scalar.update(loss3.item(), LDCTImg.size(0))
...
if __name__ == "__main__":
    ...
    result_path = './runs/UNet2D_VGG_mGPU_R1/logs/'
    save_dir = './runs/UNet2D_VGG_mGPU_R1/checkpoints/'
    ...
    vgg = vgg_feature_extractor().to('cuda:1')
    vgg.eval()

    model = UNet2d().to('cuda:0')
    ...
    if os.path.exists(save_dir) is False:
        ...
        model = load_model_model_parallel(model, checkpoint_latest)
        model = model.to('cuda:0')
